 ...
 */
export async function loadDatasetColumn(containerElement, bucketName, emptyMessage) {
  ...
  try {
    const response = await fetch(`${PROXY_URL}/api/list/${bucketName}`);
    ...
    console.log(`✅ Fetched ${files.length} files from ${bucketName}`);
